...
/**
 * Il ponte di sessione, lato guscio: la webview naviga qui col codice
 * monouso ricevuto via deep link, e ne esce con il cookie di sessione
 ...
 *
 * Monouso e a vita breve per costruzione: il codice si cancella PRIMA di
 * coniare, e un codice scaduto o già speso finisce su una pagina di
 * spiegazione, non su un errore muto.
 */
...
export async function GET(request: Request) {
  ...
  // Monouso: si brucia prima di coniare. Se due riscatti corrono, il
  // deleteMany ne premia uno solo.
  ...
  const jwt = await encode({
    token: riga.claims as Record<string, unknown>,
